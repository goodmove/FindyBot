...
import src.image_processing.impros as imp
...
class PhotoDownloader(object):
	...
	def downloadWithFaces(	self, photo_count=10, thread_count=10, show_thread_count=False,
		...
		for id in self.ids:
			...
			def downloadPack(offset, attempts, method):
				request = vkapi.getRequest(method,
					owner_id=				id,
					no_service_albums=	0,
					photo_sizes=			1,
			...
			# downloadPack(0, 5, 'photos.getAll')
			downloadPack(0, 4, 'photos.getAll')
	...
	def _download(self, link, name, check_face):
		...
		try:
			...
			return False
		if check_face:
			face = imp.detect_faces(path=name, count=1)
			if len(face) is 0:
				os.remove(name)
				return False
			else:
