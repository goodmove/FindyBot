...
def extend(face_rect, path=None, img=None, displ=None):
    """
        @descr:
            extends the face rectangle by given `displ` values, if they don't
            move the rectangle beyond image borders. Otherwise, `displ` is half
            the distance to the image borders.
        @args:
            face_rect - tuple(); x, y, w, h values of the face ractangle
    ...
    if img is None and path is None:
        raise RuntimeError('Can\'t process image. Provide either a path or an image array')
...
def resize(img, size=CONFIG['hog_conf']['resize_values'], preserve_range=True):
